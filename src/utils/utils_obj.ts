...
const getStationDetails = async (code: string) => {
    const stops = await csv().fromFile('./KMRL-Open-Data/stops.txt');
...
const find_stop_times = async (stop: string, direction: string, date: string) => {
    ...
    const trips = await csv().fromFile('./KMRL-Open-Data/trips.txt');
    const stop_times = await csv().fromFile('./KMRL-Open-Data/stop_times.txt');
...
const get_fares = async (start: string, end: string) => {
    const fare_rules = await csv().fromFile('./KMRL-Open-Data/fare_rules.txt');
    const fare_rule = _.find(fare_rules, { 'origin_id': start, 'destination_id': end });
    if (!fare_rule) {
        throw ('Fare rule not found');
    }
    const fare_attributes = await csv().fromFile('./KMRL-Open-Data/fare_attributes.txt');
